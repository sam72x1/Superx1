...
flowchart LR
    ...
    C --> D["عملية Kronos محلية على loopback"]
...
- `services/kronos_inference/`: خدمة استدلال في عملية فرعية، تحميل lazy
  للأوزان، تحقق صارم، Bearer اختياري، وحدود للطلب.
- `runner_scanner/render_supervisor.py`: يشغّل الماسح وKronos في عامل Render
  واحد، ينتظر الجاهزية، يعيد تشغيل Kronos وحده عند سقوطه، ويمرر الإيقاف
  للماسح أولًا حتى يُفرغ طابور Shadow قبل إيقاف الاستدلال.
...
- النشر الافتراضي يستخدم **عامل Render واحدًا** اسمه `runner-scanner` على
  Standard (2GB)، بدل عامل Starter وخدمة Standard إضافية. يبقى اسم الخدمة
  والقرص `/var/data` كما هما، فلا تُفقد قاعدة SQLite أو حماية منع التكرار.
- يجمع العامل ذاكرة الماسح وPyTorch؛ القياس الفعلي لـKronos وحده عند 512/18
  بلغ 445.81MiB. يقيد المشرف خيوط BLAS إلى واحد، ويبقي الاستدلال في عملية
  فرعية حتى يستمر الماسح ويعاد تشغيل Shadow وحده إذا سقط. كما يراقب `/health`
  ويعيد العملية إذا تجاوز الاستدلال 150 ثانية أو فشل الفحص ثلاث مرات متتالية.
...
- `build_runtime.py` يخبز checkout والأوزان والـtokenizer بالمراجعات المثبتة
  داخل build artifact، ويتحقق منها بـ`local_files_only=True`. يعمل runtime مع
  `HF_HUB_OFFLINE=1`؛ لا يلزم قرص cache دائم ولا تنزيل عند الإقلاع.
- داخل العامل ترتبط الخدمة بـ`127.0.0.1` فقط ولا تحتاج HTTPS أو Bearer. ينقّي
  المشرف بيئة عملية Kronos من أسرار Massive وتيليجرام والمستشار. هذا يقلل
  التعرض لكنه ليس عزل حاوية: العمليتان تعملان تحت مستخدم Unix نفسه، وهو
  التنازل المقصود مقابل عدم دفع خدمة ثانية.
- احتفظ بترخيص Kronos عند تضمين checkout في صورة نشر؛ راجع
  `services/kronos_inference/THIRD_PARTY_NOTICES.md`.
- يبقى `services/kronos_inference/Dockerfile` خيارًا لنشر حاوية منفصلة مع
  عزل أقوى إذا قُبلت تكلفتها لاحقًا. القياس الفعلي عند الإعداد الإنتاجي الكامل
  (512 شمعة، توقع 18) بلغ 445.81MiB و2.35ث للتحميل البارد و0.93ث للتوقع
  الدافئ على CPU محلي. يبقى النشر التلقائي معطلًا للعامل.
