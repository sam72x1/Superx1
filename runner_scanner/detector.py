"""كشف الأسهم الصاعدة (فوق حدّ الحركة) من Full Market Snapshot.
...
نقّي قبل قصّ الـ15 (وإلا تأخذ التشوّهات والمشتقات والسنتات مقاعد بلا تعويض):
...
- **نطاق السعر** ($1–$30): السنتات ترتفع نسبيًا أكثر فتتصدّر الفرز وتأكل
  مقاعد «أعلى 15» وتدفع الأسهم الحقيقية تحت القطع فلا تُحلَّل. نستبعدها هنا
  مبكّرًا (السعر حقيقة من السنابشوت)؛ بوّابة السعر تبقى دفاعًا مكمّلًا.
"""
...
def detect_runners(cfg: Config, snapshot: list[SnapshotEntry]) -> list[SnapshotEntry]:
    ...
    for e in snapshot:
        ...
        if not (cfg.trigger_change_pct <= e.change_pct <= cfg.max_change_pct):
            continue
        # نطاق السعر مبكّرًا: السنتات/فوق-النطاق تأكل مقاعد أعلى-15 وتدفع
        # المرشّحين الحقيقيين تحت القطع. (البوّابة تبقى دفاعًا مكمّلًا.)
        if not (cfg.price_min <= e.last_price <= cfg.price_max):
            continue
