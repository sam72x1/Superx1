...
_SYSTEM = (
    ...
    "ملاحظة: الباكتيست يعيد فحص كل سهم عند كل شمعة 5د (يحاكي المسح الحي)، فرفض "
    "RVol يعني عدم بلوغ العتبة طوال اليوم = سيولة رقيقة حقيقية لا artifact مبكّر. "
    "ميّز البوّابات الهيكلية (نوع/بورصة) عن القابلة للضبط. (3) توصيات عملية "
...
def _rule_notes(cfg: Config, res, grid) -> str:
    ...
    if f.get("considered"):
        ...
        if rr:
            ...
            if gate == "RVol":
                out.append("  ↳ رفض مشروع غالبًا: الباكتيست يعيد فحص كل سهم عند "
                           "كل شمعة 5د (مثل الحي)، فعدم بلوغ RVol=5x طوال اليوم = "
                           "سيولة رقيقة فعلًا لا artifact. راجع «قياس الظل» في التقرير "
                           "قبل أي تفكير بخفض العتبة.")
