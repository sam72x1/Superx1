...
class ClaudeClient:
    ...
    def __init__(self, api_key: str, timeout: float = 30.0,
                 on_config_error: Optional[Callable[[str], None]] = None):
        self.api_key = api_key
        self.timeout = timeout
        # يُنادى عند 401/404 (تعفّن إعداد: مفتاح باطل أو معرّف نموذج مسحوب) —
        # يوصّله main بـ monitor.raise_fault كي لا يتعطّل الذكاء بصمت (§3/التقرير).
        self.on_config_error = on_config_error

    def _flag_config_error(self, status: int, where: str, detail: str) -> None:
        """401/404 = تعفّن عقد خارجي لا خلل عابر → ERROR + عطل (إن وُصِّل)."""
        msg = f"Anthropic {where} خطأ إعداد {status}: {detail[:120]}"
        logger.error("%s", msg)
        if self.on_config_error:
            try:
                self.on_config_error(msg)
            except Exception:  # noqa: BLE001 — الإبلاغ لا يُسقط الاستدعاء
                logger.debug("تعذّر إبلاغ عطل Anthropic")
        ...
        try:
            ...
            if resp.status_code != 200:
                if resp.status_code in (401, 404):
                    self._flag_config_error(resp.status_code, "structured", resp.text)
                else:
    ...
    def chat(self, model: str, system: str, prompt: str,
        ...
        try:
            ...
            if resp.status_code != 200:
                if resp.status_code in (401, 404):
                    self._flag_config_error(resp.status_code, "chat", resp.text)
                else:
