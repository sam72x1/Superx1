    ...
    def update_outcomes(self, price_map: dict[str, float],
        ...
        with self._lock:
            ...
            for r in rows:
                ...
                # وقف الشراء الشرعي دائمًا **تحت** الأساس؛ وقفٌ عند/فوقه = مجمَّد من
                # دورة بسعر أعلى (COALESCE) فنتجاهله كي لا يُطلق hit_stop زائفًا فور
                # أوّل رصد (BUG-32: قصّة FBRX — وقف 25.9 فوق أساس 24.9). للمُنبَّه
                # عنه بعد إرساء entry_price يبقى الوقف تحت الأساس دائمًا (سليم).
                stop_valid = bool(r["stop_price"] and first > 0
                                  and r["stop_price"] < first)
