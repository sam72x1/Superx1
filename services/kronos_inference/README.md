# خدمة Kronos للاستدلال المعزول

خدمة HTTP صغيرة تفصل PyTorch وأوزان Kronos عن عامل Superx1 الرئيسي. تستخدم
مكتبة بايثون القياسية للـHTTP، ولا تشغّل CORS أو وضع debug، وتستمع افتراضيًا
على `127.0.0.1` فقط. لا تنفذ الخدمة صفقات؛ تعيد عوائد متوقعة لاستخدامها في
وضع Shadow والتقييم.
...
يضيف `render.yaml` الجذري الخدمة إلى Blueprint المشروع على خطة `standard` ذات
2GB، ويولّد `KRONOS_API_TOKEN` تلقائيًا ثم يمرره وعنوان HTTPS إلى عامل Superx1
بمراجع Render، دون كشف السر في Git. القياس الفعلي على ARM64/CPU بالإعداد
الإنتاجي الكامل (512 شمعة، توقع 18) بلغ 445.81MiB RSS و2.35ث باردًا و0.93ث
دافئًا؛ لذلك خطة 512MiB لا تترك هامشًا إنتاجيًا آمنًا. الأوزان والـtokenizer
بالمراجعات المثبتة مخبوزة داخل صورة Docker ويعمل runtime بـ
`HF_HUB_OFFLINE=1`؛ لا يلزم قرص cache دائم ولا تنزيل من Hugging Face عند
الإقلاع. يبقى النشر التلقائي معطلًا.
